...
class Actor(nn.Module):
    ...
    def _init_weights(self):
        ...
        for m in self.layers:
            if type(m) == nn.Linear:
                m.weight.data.noraml_(0.0, 1e-3)
...
class DDPG(object):
    ...
    def _sample_batch(self):
        ...
        actions = map(lambda x: x.action, batch)
    ...
    def update(self):
        ...
        mask = [1 if x else 0 for x in terminates]
        mask = totensor(mask)

        target_next_actions = self.target_actor(batch_next_states)
        target_next_value = self.target_critic(batch_next_states, target_next_actions)
